...
public class SAMLWriter {
	...
	public static Attribute buildStringAttribute(String name, String value, XMLObjectBuilderFactory builderFactory)
		...
		attr.getAttributeValues().add(attrValueFirstName);
		return attr;
	}
	...
	public static Assertion buildDefaultAssertion(SAMLInputContainer input) {
		try {
			...
			confirmationMethod.setNotOnOrAfter(now.plusMinutes(3600));

			SAMLObjectBuilder subjectConfirmationBuilder = (SAMLObjectBuilder) SAMLWriter.getSAMLBuilder()
					.getBuilder(SubjectConfirmation.DEFAULT_ELEMENT_NAME);
			SubjectConfirmation subjectConfirmation = (SubjectConfirmation) subjectConfirmationBuilder.buildObject();
			confirmationMethod
					.setRecipient("https://mihtrawellness.iv.wellsuite.com/WsWebAppSAML/AssertionConsumerService.aspx");
			DateTime confirmationTime = new DateTime();
			confirmationMethod.setNotBefore(confirmationTime);
			confirmationMethod.setNotOnOrAfter(confirmationTime.plusYears(20));
			...
			DateTime authnTime = new DateTime();
			authnStatement.setAuthnInstant(authnTime);
			authnStatement.setSessionIndex(input.getSessionId());
			authnStatement.setSessionNotOnOrAfter(authnTime.plus(60));
